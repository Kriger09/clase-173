...
AFRAME.registerComponent("markerhandler", {
  ...
  handleMarkerFound: function (dishes, markerId) {
    ...
    if (dish.unavailable_days.includes(days[todaysDay])) {
      ...
      var reviewPlane = document.querySelector(`#feedback-plane-${dish.id}`);
  ...
  handleRatings: async function (dish) {
    ...
    var order = await this.getOrderSummary(tNumber)
    var current_orders = Object.keys(order.current_orders)
    ...
    if (current_orders.length > 0 && current_orders == dish.id) {
      ...
      swal({
        icon: "warning",
        title: "¡Ups!",
        text: "No se encuentra el platillo que ha pedido en su orden",
        timer: 2500,
        buttons: false
      });
    }
